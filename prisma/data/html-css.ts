...
<div class="info-note">
  <p>Для многостраничных сайтов важно, чтобы каждая страница имела свой тег <code>&lt;title&gt;</code>, который соответствует содержимому именно этой части сайта.</p>
</div>
...
<div class="info-note">
  <p>Кодировку страницы необходимо указать, чтобы браузер знал, какие символы используются, и мог корректно отобразить текст. Если этого не сделать или установить неправильную кодировку, вместо символов браузер может отобразить иероглифы.</p>
</div>
...
`,
    ...
    },
    {
      ...
      order: 1,
      homework: {
        id: "css-homework-portfolio",
        title: "Домашнее задание: Портфолио",
        description:
          "Создание стилей для главной страницы и страницы портфолио.",
        figmaUrl:
          "https://www.figma.com/design/65TsLGyL0fDJmc084cEoN5/Web-Studio--Version-2.1---Copy-?node-id=1-94&p=f&t=ZZXCCdiiUtiuXv6I-0",
        acceptanceCriteria: [
          {
            id: "markup-homework",
            title: "Разметка",
            items: [
              "Разметка секции `Our Portfolio` набрана в файле `index.html`. Выполнена HTML-разметка всех элементов макета.",
              "Теги использованы в соответствии с их семантическим смыслом.",
              "HTML проходит проверку валидатором без ошибок.",
              "Имена классов описательные и понятные другому разработчику.",
              "Имена классов не содержат заглавных букв, пробелов, транслита и названий тегов. Имена содержат только буквы и слова английского языка. Если имя класса состоит из нескольких слов, они разделяются дефисом.",
              "У тегов `<img>` указаны атрибуты размеров, как минимум `width`.",
              "Изображения экспортированы из макета в формате `jpg`.",
              "Группы однотипных элементов собраны в списки `<ul>`.",
              "Все необходимые шрифты по макету и их вариации (вес и начертание) подключены из сервиса Google Fonts одной ссылкой. Необходимый вес для Raleway - 700, а для Roboto - 400, 500, 700.",
            ],
          },
          {
            id: "styling-homework",
            ...
            items: [
              ...
              "Интерактивные элементы (кнопки и ссылки), при наведении мышью или фокусе с клавиатуры, имеют активное состояние, указанное в Style guide (изменение цвета).",
              "Текст контактов в хедере меняет цвет при ховере и фокусе.",
              "Для элемента `<body>` задано свойство `font-family` с доминантным шрифтом на макете `Roboto`.",
              "Указаны альтернативные варианты шрифта и тип семейства (без засечек) в конце списка `font-family` у элемента `<body>`.",
              ...
              "Высота строки (свойство `line-height`) всех текстовых элементов точно соответствует значениям из макета и задана как множитель, а не в `px`.",
              "Цвет (свойства `color` и `background-color`) всех элементов точно соответствует значениям из макета.",
              "Вес шрифта (свойство `font-weight`) всех текстовых элементов точно соответствует значениям из макета. Вес шрифта явно указан только, если значение в макете отличается от стандартного для этого элемента в браузере.",
              "Кнопкам задано свойство `cursor` со значением `pointer`.",
              "В стилях не повторяются значения свойств, которые заданы браузером по умолчанию. Например, ссылкам не нужно указывать `cursor: pointer`, а абзацам `font-style: normal` или `font-weight: 400`.",
            ],
          },
          {
            id: "project-setup-homework",
            title: "Проект",
            items: [
              "В корне проекта есть папка `images` с изображениями.",
              "В корне проекта есть папка `css` с файлом стилей.",
              "Все стили написаны в одном файле `styles.css`, который находится в папке `css`.",
              "В названиях файлов отсутствуют заглавные буквы, пробелы и транслит. Названия содержат только буквы и слова английского языка.",
              "Исходный код отформатирован с помощью `Prettier`.",
              "Все изображения и текстовый контент взяты из макета.",
              "Подключен нормализатор стилей `modern-normalize`.",
              "Все растровые изображения оптимизированы с использованием `squoosh`.",
              "Код написан с соблюдением руководства.",
            ],
          },
        ],
      },
...
`,
        ...
        {
          id: "css-homework-2-plan",
          title: "План выполнения ДЗ 2",
          videoUrl: null,
          order: 9,
          content: `
<h3>С чего начать выполнение ДЗ 2?</h3>

<p>Начнем с создания репозитория!</p>
...
<div class="info-note">
  <p><strong>! Нужно вспомнить, как создать новый репозиторий на основе предыдущего?</strong></p>
  <p>Вернись в прекурс и открой раздел <strong>Основы Git и Github</strong>, а в нем: <strong>Создаем новый репозиторий на основе предыдущего.</strong></p>
</div>

<ul class="list-disc">
  <li>Когда репозиторий <code>goit-markup-hw-02</code> создан, его нужно склонировать и открыть в VSCode.</li>
</ul>

<h3>Разметка и новые папки проекта</h3>

<p>Дальше двигаемся по такому алгоритму:</p>

<ul class="list-disc">
  <li>Добавь html-разметку секции <strong>Portfolio</strong>.</li>
  <li>Не забудь оптимизировать изображения и добавь их в папку <code>images</code>.</li>
  <li>Создай папку <code>css</code>. Внутри этой папки создай файл <code>styles.css</code>. Файл нужно подключить в файлах html с помощью тега <code>&lt;link&gt;</code>.</li>
</ul>

<h3>Анализ макета</h3>

<p>Уже было в первом модуле? Да, но теперь ты знаешь много нового про CSS! Проанализируй макет, чтобы определить:</p>

<ul class="list-disc">
  <li>Какие шрифты используются?</li>
</ul>

<p>Найди нужные шрифты на сайте Google Fonts и сформируй ссылку. Подсказка: оба шрифта нужно подключать одной ссылкой. Добавь ссылку в html-файлы.</p>

<ul class="list-disc">
  <li>Какие цвета используются в макете?</li>
</ul>

<p>С этим поможет Style Guide.</p>

<h3>Порядок добавления стилей:</h3>

<ul class="list-disc">
  <li>Пропиши стили для <code>body</code> (<code>font-family</code>, <code>color</code> и <code>background-color</code>)</li>
  <li>Пропиши CSS-правила для сброса дефолтного подчеркивания для ссылок и маркеров в списках.</li>
</ul>

<h3>Работа со стилями элементов</h3>

<p>Когда предыдущие пункты выполнены, пора начать работать со стилями элементов. Помни, что мы движемся слева направо и сверху вниз по макету.</p>
<p>Для каждого элемента:</p>

<ul class="list-disc">
  <li>Выбираем элемент, с которым будем работать,</li>
  <li>В файле <code>html</code> добавляем атрибут <code>class</code> с именем класса,</li>
  <li>В файле <code>css</code> обращаемся к этому элементу по селектору класса и прописываем необходимые свойства (цвет, свойства шрифта).</li>
</ul>

<p>Перед тем, как сдать готовое ДЗ 2, проверь, что оно соответствует всем <strong>Критериям приема работы наставником</strong>, которые перечислены во вкладке <strong>Домашнее задание</strong>.</p>
`,
          challenges: [],
        },
      ],
    },
    {
      ...
      topics: [
        {
          id: "css-box-model-program",
          title: "Программа модуля",
          videoUrl: null,
          order: 1,
          content: `
<p>Друг! Приближаемся к середине блока HTML + CSS. Давай посмотрим, чем займемся на этой неделе!</p>

<h3>Цели модуля 3:</h3>

<ul class="list-disc">
  <li>Разобраться, что такое блочная модель и для чего она используется</li>
  <li>Научиться настраивать геометрию элемента</li>
  <li>Понять, как взаимодействуют поля (margin и padding) разных элементов</li>
  <li>Узнать об основных концепциях технологии Flexbox</li>
  <li>Научиться настраивать свойства flex-контейнера и flex-элементов</li>
  <li>Настраивать отдельные элементы с помощью псевдоклассов <code>:first-child</code>, <code>:last-child</code>, <code>:not()</code> и <code>:nth-child(an + b)</code>.</li>
</ul>

<p>В этом модуле тебя ждет много, реально много примеров живого кода. И это не просто посмотреть на этот код, как на картинку. Это намно-о-ого более крутой инструмент.</p>

<h3>Что же это такое?</h3>

<p>Это песочница, в которой можно работать с HTML, CSS и JavaScript. То есть ты одновременно видишь HTML-разметку, CSS-файл и отображение всего этого в браузере.</p>

<h3>Выглядит он так:</h3>

<div class="codepen-container">
  <iframe height="300" style="width: 100%;" scrolling="no" title="Module 3 Program" src="https://codepen.io/goit-academy/embed/mdEJYPN?default-tab=html%2Cresult" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true">
    See the Pen <a href="https://codepen.io/goit-academy/pen/mdEJYPN">Module 3 Program</a> by goit-academy (<a href="https://codepen.io/goit-academy">@goit-academy</a>) on <a href="https://codepen.io">CodePen</a>.
  </iframe>
</div>

<p>Инструкции есть почти к каждому живому примеру с советами, на что стоит обратить внимание.</p>

<div class="info-note">
  <p><strong>! Используй этот инструмент на 100%</strong></p>
  <p>1. Кликай на логотип codepen в верхнем правом углу и переходи на страницу.</p>
  <p>2. На странице смело меняй параметры, чтобы увидеть, как всё работает, и “что будет, если я...” Нет, ты ничего не сломаешь, все изменения локальны.</p>
</div>
`,
          challenges: [],
        },
        {
          ...
          title: "Блочная модель элемента",
          videoUrl: null,
          order: 2,
          content: `
<p>Веб-страницы — это далекие потомки газет.</p>
<p>Нужны доказательства? Обрати внимание на заголовки, колонки, секции, а особенно на их расположение относительно друг друга. Разные по размеру блоки, расположенные на определенном расстоянии друг от друга, заполняют пространство страницы. Это ведь прямое наследование логики газетной страницы.</p>

<p>Опытные разработчики сразу видят страницу как совокупность блоков. Это помогает им быстро и эффективно анализировать макет. А скоро и ты научишься!</p>

<h2>Блочная модель</h2>

<p>Блочная модель (англ. box model) определяет расположение элементов на странице и их взаимодействие между собой. Независимо от того, как элемент визуально выглядит на странице — это, прежде всего, прямоугольный контейнер. У этого контейнера есть 4 области: контент элемента (внутри), внутреннее поле (паддинг), рамка и внешний отступ (маржин).</p>

<div class="image-container">
  <img src="/images/module-3/box-model-diagram.png" alt="Схема блочной модели" class="img-responsive" />
</div>

<ul class="list-disc">
  <li><strong>Область контента (content)</strong> — это контент элемента, например текст, изображение, видео или вложенные элементы. По <strong>умолчанию</strong> высота (height) области контента определяется контентом, ширина (width) — типом элемента (блочный или строчный).</li>
  <li><strong>Паддинг (англ. padding, внутреннее поле)</strong> — это расстояние между контентом элемента и его рамкой (border). Используется для декоративных эффектов. Нельзя указывать отрицательные значения.</li>
  <li><strong>Рамка (border)</strong> — граница элемента. По <strong>умолчанию</strong> её ширина равна нулю. Если цвет рамки не установлен, она принимает цвет основного контента, например текста.</li>
  <li><strong>Маржин (англ. margin, внешний отступ)</strong> — создает отступ снаружи элемента, от внешней границы (рамки) до соседних элементов, тем самым разделяя их на странице. Маржину можно указывать отрицательные значения. Фон элемента на внешний отступ не распространяется.</li>
</ul>

<h3>Изучи схему</h3>

<div class="image-container">
  <img src="/images/module-3/box-model-zones.png" alt="Зоны блочной модели" class="img-responsive" />
</div>
`,
          challenges: [],
        },
